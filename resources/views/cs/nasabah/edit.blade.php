...
@section('content')
    <div class="flex justify-center items-start py-10 w-full  min-h-[calc(100vh-80px)]">
        <div class="w-full max-w-xl px-10 py-10 mx-auto border shadow-xl drop-shadow-xl ">
            @if ($errors->any())
                <div class="px-4 py-2 bg-red-200 rounded-md alert alert-error">
                    @foreach ($errors->all() as $error)
                        <p>{{ $error }}</p>
                    @endforeach
                </div>
            @endif
            <h2 class="inline-block text-xl font-semibold text-center text-slate-700">Edit Nasabah Baru</h2>
            <form action="{{route('changed.nasabah')}}" class="flex flex-col gap-3 ">
                @csrf
                ...
                <label class="flex flex-col justify-center gap-2">
                    ...
                    <div class="text-xs text-red-500" id="error-message"></div>
                </label>
                <label class="flex flex-col justify-center gap-2">
                ...
                <div class="grid w-full grid-cols-2 gap-5 mt-5 ">
